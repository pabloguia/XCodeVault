...
public struct RuntimeOperations: Sendable {
    ...
    public func delete(identifier: String, keepAsset: Bool = false, dryRun: Bool = false) throws -> CommandResult {
        ...
        if !dryRun { try journal.record(id: op, kind: .runtimeDelete, state: .started, summary: "simctl runtime delete \(identifier)\(keepAsset ? " --keep-asset" : "")") }
        let r = try runner.run(Tools.xcrun, args, environment: env)
        if !dryRun {
            try journal.record(id: op, kind: .runtimeDelete, state: r.succeeded ? .completed : .failed, summary: r.succeeded ? "deleted \(identifier)" : "failed: \(r.stderr.trimmingCharacters(in: .whitespacesAndNewlines))")
        }
    ...
    public func preflightExport(_ req: ExportRequest, freeBytesAtDestination: UInt64?) throws -> [String] {
        ...
        guard caps.downloadPlatform, caps.exportPath else { throw RuntimeOperationError("Xcode \(xcode.version) does not support `-downloadPlatform … -exportPath` (feature-detected).") }
        guard ["iOS", "watchOS", "tvOS", "visionOS"].contains(req.platform) else { throw RuntimeOperationError("Unknown platform \(req.platform); use iOS, watchOS, tvOS or visionOS.") }
        if req.buildVersion != nil && !caps.buildVersion { throw RuntimeOperationError("This Xcode rejects -buildVersion (feature-detected); omit it to get the matching runtime.") }
        if let a = req.architectureVariant {
            ...
            if a == "arm64" && !host.isAppleSilicon { w.append("arm64-only runtimes cannot run on an Intel Mac; exporting anyway (useful only for an Apple Silicon machine).") }
        ...
        guard FileManager.default.fileExists(atPath: req.destination, isDirectory: &isDir), isDir.boolValue else { throw RuntimeOperationError("Destination \(req.destination) is not an existing directory.") }
        guard FileManager.default.isWritableFile(atPath: req.destination) else { throw RuntimeOperationError("Destination \(req.destination) is not writable.") }
        if let free = freeBytesAtDestination, free < 12_000_000_000 { w.append("Only \(ByteCount.format(free)) free at the destination; runtime images are 5–25 GB.") }
        if host.dataVolumeFreeBytes < 15_000_000_000 {
            w.append("Only \(ByteCount.format(host.dataVolumeFreeBytes)) free on the internal volume. Downloads may stage internally before export (E11 — unverified); watch for ENOSPC.")
        }
        return w
    }
    ...
    public func export(_ req: ExportRequest) throws -> CommandResult {
        ...
        try journal.record(id: op, kind: .runtimeExport, state: r.succeeded ? .completed : .failed,
                           summary: r.succeeded ? "exported \(req.platform) to \(req.destination)" : "failed: \(r.stderr.trimmingCharacters(in: .whitespacesAndNewlines).suffix(300))", paths: [req.destination])
    ...
    public func preflightImport(dmg: String) throws -> [String] {
        guard xcode.capabilities.importPlatform else { throw RuntimeOperationError("Xcode \(xcode.version) does not support -importPlatform (feature-detected).") }
        guard let attrs = try? FileManager.default.attributesOfItem(atPath: dmg), let size = attrs[.size] as? UInt64 else { throw RuntimeOperationError("\(dmg) does not exist.") }
        ...
        if host.dataVolumeFreeBytes < need {
            throw RuntimeOperationError("Installing needs internal staging space: image is \(ByteCount.format(size)), only \(ByteCount.format(host.dataVolumeFreeBytes)) free (want ≥ \(ByteCount.format(need))). Free space first (`xcodevaultctl clean`).")
        }
        if host.dataVolumeFreeBytes < size * 4 { w.append("Internal free space is tight for staging (\(ByteCount.format(host.dataVolumeFreeBytes))); the reported requirement is ~40 GB for a 9–12 GB runtime (E11, unverified).") }
        return w
    }
    ...
    public func importRuntime(dmg: String) throws -> CommandResult {
        ...
        let r = try runner.run(xcodebuild, args, environment: env)
        try journal.record(id: op, kind: .runtimeImport, state: r.succeeded ? .completed : .failed,
    ...
    public static func library(at dir: String) throws -> [RuntimeInstaller] {
        ...
        for n in names where n.lowercased().hasSuffix(".dmg") {
            ...
            out.append(RuntimeInstaller(path: p, fileName: n, sizeBytes: (attrs[.size] as? UInt64) ?? 0,
