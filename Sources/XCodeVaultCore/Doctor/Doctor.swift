...
public struct Finding: Sendable, Codable, Equatable, Identifiable {
    public enum Severity: String, Sendable, Codable, Comparable {
        ...
        private var rank: Int { switch self { case .info: 0; case .warning: 1; case .error: 2; case .critical: 3 } }
    }
...
public struct Doctor: Sendable {
    ...
    func checkForbiddenSymlinks() -> [Finding] {
        ...
        for t in CatalogRules.neverSymlink {
            ...
            case "~/Library/Developer/CoreSimulator": why = "Breaks the Simulator's Files app (share/save/create folder) even when the target is on the same disk (H5, Aug 2025 report)."
            ...
            out.append(Finding(id: "forbidden-symlink:\(t)", severity: .critical, title: "\(t) is a symlink → \(target)",
                               detail: why + " This configuration was not created by XCodeVault (it is exactly what mac-ssd-rescue creates).",
                               path: p, remediation: "Move the data back to \(p) as a real directory (copy with `ditto`, verify, then replace the symlink). XCodeVault will offer a verified restore in a later milestone.",
    ...
    func checkBrokenSymlinks() -> [Finding] {
        ...
        while let rel = e.nextObject() as? String {
            ...
            if !FileManager.default.fileExists(atPath: resolved) {
                out.append(Finding(id: "broken-symlink:\(rel)", severity: .error, title: "Broken symlink under ~/Library/Developer",
                                   detail: "\(p) → \(target) (target missing). Typical of a relocation tool whose destination volume is not mounted.",
                                   path: p, remediation: "Reconnect the destination volume, or restore the directory. Do not let Xcode recreate it as a local directory first — that creates shadow data.",
                                   evidence: "docs/process/PRIOR_ART.md"))
            } else if target.hasPrefix("/Volumes/") {
                out.append(Finding(id: "external-symlink:\(rel)", severity: .warning, title: "Symlink into /Volumes under ~/Library/Developer",
                                   detail: "\(p) → \(target). Absolute symlinks into /Volumes break when the volume mounts under a different name (e.g. 'Name 1') or is absent.",
    ...
    func checkPriorToolLeftovers(volumes: [Volume]) -> [Finding] {
        ...
        for v in volumes where v.isExternal || v.isDiskImage {
            ...
            if FileManager.default.fileExists(atPath: candidate, isDirectory: &isDir), isDir.boolValue {
                ...
                out.append(Finding(id: "prior-tool:mac-ssd-rescue:\(v.id)", severity: .warning,
                                   title: "mac-ssd-rescue data found on \(v.volumeName)",
                                   detail: "\(candidate) contains: \(contents.sorted().joined(separator: ", ")). If ~/Library/Developer no longer links here, these are stale duplicates; if it does, they are live data with a documented-broken configuration (H5).",
                                   path: candidate, remediation: "Compare with the local copies before deleting anything. XCodeVault `verify` will diff them in a later milestone.",
                                   evidence: "docs/process/PRIOR_ART.md"))
            }
        }
        return out
    ...
    func checkFreeSpace(host: HostEnvironment) -> [Finding] {
        ...
        return [Finding(id: "low-free-space", severity: sev, title: "Low free space on the internal volume: \(ByteCount.format(free))",
                        detail: "Simulator runtime installs stage on the internal volume and reportedly need ~40 GB free even when the installer is elsewhere; Xcode itself needs headroom for indexes and builds.",
                        path: host.homeDirectory, remediation: "Run `xcodevaultctl scan` and clean regenerable categories first (DerivedData, old Device Support, unused runtimes via `simctl runtime delete`).",
    ...
    func checkRuntimeRegistry(runtimes: [SimulatorRuntime]) -> [Finding] {
        ...
        for r in runtimes {
            ...
            if let mp = r.mountPath, r.state == "Ready", !MountStatus.isMountPoint(mp) {
                out.append(Finding(id: "runtime-not-mounted:\(r.identifier)", severity: .error, title: "Runtime \(name) is Ready but not mounted",
                                   detail: "Registry says Ready; nothing is mounted at \(mp). Xcode will not see this runtime.",
                                   path: mp, remediation: "`xcrun simctl runtime verify \(r.identifier)` then restart CoreSimulator (`launchctl kickstart -k`) or reboot. Never remount with Disk Utility (Apple DTS).",
                                   evidence: "docs/research/FINDINGS-2026-09-05.md §F1"))
            }
            if let sig = r.signatureState, sig != "Verified" {
                out.append(Finding(id: "runtime-signature:\(r.identifier)", severity: .error, title: "Runtime \(name) signature state: \(sig)",
            ...
            if let p = r.path, !FileManager.default.fileExists(atPath: p) {
                out.append(Finding(id: "runtime-image-missing:\(r.identifier)", severity: .error, title: "Runtime \(name) image file missing",
    ...
    func checkStrandedInbox() -> [Finding] {
        ...
        for dir in ["/Library/Developer/CoreSimulator/Cryptex/Images/Inbox", "/Library/Developer/CoreSimulator/Images/Inbox"] {
            ...
            for n in names where !n.hasPrefix(".") {
                ...
                out.append(Finding(id: "stranded-inbox:\(n)", severity: .warning, title: "Stranded runtime download: \(n) (\(ByteCount.format(size)))",
    ...
    func checkOrphanedAssets(runtimes: [SimulatorRuntime]) -> [Finding] {
        var out: [Finding] = []
        let referenced = Set(runtimes.compactMap { $0.path }.compactMap { p -> String? in
        ...
        for store in stores where store.hasPrefix("com_apple_MobileAsset_") && store.hasSuffix("SimulatorRuntime") {
            ...
            for e in entries where e.hasSuffix(".asset") {
                ...
                if !referenced.contains(p) {
                    let size = DiskUsage.measure(p)?.allocatedBytes ?? 0
                    out.append(Finding(id: "orphan-asset:\(e)", severity: .warning, title: "Runtime asset not referenced by any runtime (\(ByteCount.format(size)))",
                                       detail: "\(p) exists in the MobileAsset store but `simctl runtime list` references no runtime backed by it. Likely a NeverCollected orphan (F1).",
                                       path: p, remediation: "Do not delete by hand (under /System). Try `xcrun simctl runtime delete all --dry-run` to see whether simctl knows it; otherwise it is an Apple bug to report.",
    ...
    func checkUnavailableDevices(devices: [SimulatorDevice]) -> [Finding] {
        ...
        return [Finding(id: "unavailable-devices", severity: .warning, title: "\(bad.count) simulator device(s) unavailable (\(ByteCount.format(bytes)))",
    ...
    func checkDerivedDataLocation(volumes: [Volume]) -> [Finding] {
        ...
        if let vol, vol.isExternal || vol.isDiskImage || loc.hasPrefix("/Volumes/") {
            return [Finding(id: "deriveddata-external", severity: .info, title: "DerivedData is on an external volume (\(loc))",
                            detail: "Apple's supported relocation, but framework unit tests are reported to fail loading test bundles from external volumes (F4). E2 on this machine: see COMPATIBILITY_MATRIX.md.",
    ...
    func checkXcodeSelect(xcodes: [XcodeInstallation]) -> [Finding] {
        ...
        return [Finding(id: "xcode-select", severity: .warning, title: "xcode-select does not point at any installed Xcode",
    ...
    func checkOS(host: HostEnvironment) -> [Finding] {
        ...
        return [Finding(id: "unsupported-macos", severity: .error, title: "macOS \(host.macOSVersion) is below the supported minimum (14.0)",
