...
struct Clean: ParsableCommand {
    ...
    func run() throws {
        ...
        for a in plan.actions {
            print("  \(TextRendererPad.pad(ByteCount.format(a.bytes), 10)) \(TextRendererPad.pad(a.categoryName, 34)) \(a.requiresRoot ? "[root — helper needed] " : "")\(a.isExperimental ? "(exp.) " : "")\(a.path)")
        }
...
extension Runtime {
    ...
    struct Delete: ParsableCommand {
        static let configuration = CommandConfiguration(abstract: "Delete an installed runtime via `simctl runtime delete` (the only supported way; frees the MobileAsset store).")
        ...
        func run() throws {
            ...
            if !dryRun && !yes { throw ValidationError("Deleting a runtime removes 5–25 GB that must be re-downloaded or re-imported. Pass --yes to confirm, or --dry-run.") }
    ...
    struct Export: ParsableCommand {
        static let configuration = CommandConfiguration(abstract: "Download a runtime installer to a Runtime Library directory (`xcodebuild -downloadPlatform … -exportPath`).")
    ...
    struct Library: ParsableCommand {
        static let configuration = CommandConfiguration(abstract: "List runtime installers in a Runtime Library directory and whether each installed runtime has one.")
        @OptionGroup var global: GlobalOptions
        ...
        func run() throws {
            ...
            try emit(rows, json: global.json) {
                ...
                for i in lib { o += "  \(TextRendererPad.pad(ByteCount.format(i.sizeBytes), 10)) \(TextRendererPad.pad(i.platform ?? "?", 9)) \(TextRendererPad.pad(i.version ?? "?", 7)) \(i.fileName)\n" }
                o += "\nInstalled runtimes:\n"
                for r in rts {
                    let m = RuntimeOperations.installer(for: r, in: lib)
                    o += "  \(TextRendererPad.pad(r.platformName, 9)) \(TextRendererPad.pad(r.version ?? "?", 7)) \(ByteCount.format(r.sizeBytes ?? 0))  \(m != nil ? "installer in library ✓ (can be offloaded)" : "NO installer in library — export first")\n"
                }
                return o
            }
        }
    ...
    struct Offload: ParsableCommand {
        static let configuration = CommandConfiguration(abstract: "Delete an installed runtime only if its installer already sits in the Runtime Library (export first if not). Two-phase, journaled.")
        ...
        func run() throws {
            let (x, h) = try selectedXcode()
            let rts = try SimulatorDiscovery.runtimes()
            guard let rt = rts.first(where: { $0.identifier == identifier }) else { throw ValidationError("No installed runtime with identifier \(identifier).") }
            let lib = try RuntimeOperations.library(at: library)
            guard let inst = RuntimeOperations.installer(for: rt, in: lib) else {
                throw ValidationError("No installer for \(rt.platformName) \(rt.version ?? "?") in \(library). Run `xcodevaultctl runtime export \(rt.platformName == "iphone" ? "iOS" : rt.platformName) --to \(library)` first — the runtime is NOT deleted.")
            }
            ...
            print("Installer verified: \(inst.fileName) (\(ByteCount.format(inst.sizeBytes))).")
            guard yes else { print("Would delete runtime \(rt.runtimeIdentifier ?? identifier) (\(ByteCount.format(rt.sizeBytes ?? 0))). Pass --yes to proceed."); return }
            let ops = RuntimeOperations(xcode: x, host: h)
            let op = UUID().uuidString
            try ops.journal.record(id: op, kind: .runtimeOffload, state: .started, summary: "offload \(identifier) (installer \(inst.path))", paths: [inst.path])
            let r: CommandResult
            do { r = try ops.delete(identifier: identifier) } catch {
                try ops.journal.record(id: op, kind: .runtimeOffload, state: .failed, summary: "offload \(identifier) failed: \(error)", paths: [inst.path]); throw error
            }
...
struct Locations: ParsableCommand {
    static let configuration = CommandConfiguration(abstract: "Xcode ▸ Settings ▸ Locations (DerivedData, Archives) — Apple's supported relocation.",
                                                    subcommands: [Show.self, SetDerivedData.self, ResetDerivedData.self, SetArchives.self, ResetArchives.self, SetCompilationCache.self, ResetCompilationCache.self], defaultSubcommand: Show.self)
    ...
    struct SetDerivedData: ParsableCommand {
        static let configuration = CommandConfiguration(commandName: "set-derived-data", abstract: "Point DerivedData at a directory (IDECustomDerivedDataLocation; reproduced on Xcode 26.5, E8b). Experimental until the matrix is complete.")
        @Argument(help: "Absolute path of an existing, writable directory.") var path: String
        @Flag(name: .customLong("i-understand-tests-may-fail"), help: "Acknowledge that xcodebuild test cannot load test bundles from a physical external volume (E2).") var acknowledge = false
        func run() throws {
            let volumes = (try? VolumeDiscovery.mountedVolumes()) ?? []
            let warnings = try XcodeLocations.preflightDerivedData(path: path, volumes: volumes, xcodeRunning: CleanExecutor.xcodeIsRunning(), acknowledgeExternalTests: acknowledge)
            for w in warnings { print("! \(w)") }
    ...
    struct SetArchives: ParsableCommand {
        static let configuration = CommandConfiguration(commandName: "set-archives", abstract: "Point new Archives at a directory (IDECustomDistributionArchivesLocation; reproduced on Xcode 26.5, status: probable/experimental). Existing archives are not moved — use `externalize --category archives`.")
        @Argument(help: "Absolute path of an existing, writable directory.") var path: String
        func run() throws {
            let volumes = (try? VolumeDiscovery.mountedVolumes()) ?? []
    ...
    struct SetCompilationCache: ParsableCommand {
        static let configuration = CommandConfiguration(commandName: "set-compilation-cache", abstract: "Point the Xcode 26 compilation cache at a directory (IDECustomCompilationCacheLocation). Experimental.")
        @Argument(help: "Absolute path of an existing, writable directory.") var path: String
        @Flag(name: .customLong("i-understand-tests-may-fail"), help: "Acknowledge the E2 external-volume caveat (build products may be served from here).") var acknowledge = false
        func run() throws {
            let xcodes = XcodeDiscovery.discover(detectCapabilities: false)
            guard let x = xcodes.first(where: \.isSelected) ?? xcodes.first, x.majorVersion >= 26 else { throw ValidationError("The compilation cache location setting exists in Xcode 26+; selected Xcode is older.") }
            let volumes = (try? VolumeDiscovery.mountedVolumes()) ?? []
            for w in try XcodeLocations.preflightDerivedData(path: path, volumes: volumes, xcodeRunning: CleanExecutor.xcodeIsRunning(), acknowledgeExternalTests: acknowledge) { print("! \(w)") }
...
struct JournalCommand: ParsableCommand {
    static let configuration = CommandConfiguration(commandName: "journal", abstract: "Show the operation journal (every change XCodeVault made, and any interrupted operation).")
    @OptionGroup var global: GlobalOptions
    ...
    func run() throws {
        ...
        try emit(Out(entries: Array(entries), interrupted: interrupted), json: global.json) {
            ...
            for e in entries { o += "  \(e.sequence)  \(f.string(from: e.timestamp))  \(TextRendererPad.pad(e.kind.rawValue, 19)) \(TextRendererPad.pad(e.state.rawValue, 10)) \(e.summary)\n" }
            if !interrupted.isEmpty { o += "\n! Interrupted operations (started, never completed):\n"; for e in interrupted { o += "  \(e.id)  \(e.kind.rawValue)  \(e.summary)\n" } }
            return o
        }
    }
}
