...
extension Runtime {
    ...
    struct Offload: ParsableCommand {
        ...
        func run() throws {
            ...
            let ops = RuntimeOperations(xcode: x, host: h)
            try ops.journal.record(kind: .runtimeOffload, state: .started, summary: "offload \(identifier) (installer \(inst.path))", paths: [inst.path])
            let r = try ops.delete(identifier: identifier)
            try ops.journal.record(kind: .runtimeOffload, state: .completed, summary: "offloaded \(identifier)", paths: [inst.path])
            print(r.stdout + r.stderr)
...
struct Locations: ParsableCommand {
    static let configuration = CommandConfiguration(abstract: "Xcode ▸ Settings ▸ Locations (DerivedData, Archives) — Apple's supported relocation.",
                                                    subcommands: [Show.self, SetDerivedData.self, ResetDerivedData.self], defaultSubcommand: Show.self)
    struct Show: ParsableCommand {
        @OptionGroup var global: GlobalOptions
        func run() throws {
            let l = XcodeLocations.read()
            try emit(l, json: global.json) {
                "DerivedData:          \(l.derivedData ?? "(default: ~/Library/Developer/Xcode/DerivedData)")\nBuild location style: \(l.buildLocationStyle ?? "(default)")\nArchives:             \(l.archives ?? "(default: ~/Library/Developer/Xcode/Archives) — key unverified, read-only")\n"
            }
        }
    }
    struct SetDerivedData: ParsableCommand {
        static let configuration = CommandConfiguration(commandName: "set-derived-data", abstract: "Point DerivedData at a directory (IDECustomDerivedDataLocation; verified on Xcode 26.5, E8b). Experimental until the matrix is complete.")
