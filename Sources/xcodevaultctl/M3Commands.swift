...
struct Vault: ParsableCommand {
    static let configuration = CommandConfiguration(abstract: "Experimental. Register and verify external vault volumes (identified by UUID + sentinel, never by name).",
                                                    subcommands: [Init.self, Status.self, Forget.self], defaultSubcommand: Status.self)
    struct Init: ParsableCommand {
        static let configuration = CommandConfiguration(abstract: "Register a mounted external APFS volume as a vault (creates <mount>/XcodeVault and a sentinel).")
        @Argument(help: "Mount point, e.g. /Volumes/MyDrive") var mountPoint: String
        func run() throws {
            let vols = try VolumeDiscovery.mountedVolumes()
            guard let v = vols.first(where: { $0.mountPoint == mountPoint }) else { throw ValidationError("No mounted volume at \(mountPoint). See `xcodevaultctl volumes`.") }
...
struct Externalize: ParsableCommand {
    static let configuration = CommandConfiguration(abstract: "Copy a cold-storage category (Archives) to a verified vault volume with deep verification. The source is kept unless --remove-source-after-verify is given.",
                                                    discussion: "Experimental: labeled so until the Definition of Done is met. Never merges into existing vault data; never removes a source without re-verification.")
    @OptionGroup var global: GlobalOptions
    ...
    @Flag(name: .customLong("i-confirm-deleting-non-regenerable-data"), help: "Required with --remove-source-after-verify for Archives.") var confirmNonRegenerable = false
    ...
    func run() throws {
        ...
        if !apply {
            try emit(Out(plan: plan, outcome: nil), json: global.json) { "Plan: copy \(ByteCount.format(plan.sourceBytes)) (\(plan.sourceFiles) files) \(plan.source) → \(plan.destination), deep verify. Re-run with --apply.\n" }
            return
        }
        print("Copying \(ByteCount.format(plan.sourceBytes)) → \(plan.destination) …")
        var outcome = try engine.copyAndVerify(plan)
        print("Verified: \(outcome.verification.sourceFiles) files, \(outcome.verification.hashedFiles) hashed, \(ByteCount.format(outcome.verification.destinationBytes)). Source intact.")
        if removeSourceAfterVerify {
            outcome = try engine.removeSource(outcome, confirmNonRegenerable: confirmNonRegenerable)
            print("Source removed after re-verification. Restore with: xcodevaultctl restore --category \(category) --vault \(vault) --name \"\((plan.source as NSString).lastPathComponent)\" --to \"\(plan.source)\"")
        }
...
struct Restore: ParsableCommand {
    static let configuration = CommandConfiguration(abstract: "Experimental. Copy a vault entry back to its original location with deep verification. Never overwrites; destination must be under the category's standard path.")
    @OptionGroup var global: GlobalOptions
    ...
    func run() throws {
        ...
        print("Restored and verified: \(outcome.verification.sourceFiles) files, \(ByteCount.format(outcome.verification.destinationBytes)). The vault copy is kept.")
        if global.json { print(try JSONOutput.encode(outcome)) }
    }
}

struct Migration: ParsableCommand {
    static let configuration = CommandConfiguration(abstract: "Experimental. Inspect or abort interrupted migrations.", subcommands: [Status.self, Abort.self], defaultSubcommand: Status.self)
    struct Status: ParsableCommand {
        @OptionGroup var global: GlobalOptions
        func run() throws {
            let interrupted = try Journal().interrupted().filter { $0.kind == .migration }
            try emit(interrupted, json: global.json) {
                interrupted.isEmpty ? "No interrupted migrations.\n" : interrupted.map { "INTERRUPTED \($0.id)  \($0.summary)  paths: \($0.paths.joined(separator: " → "))\n" }.joined()
            }
        }
    }
    struct Abort: ParsableCommand {
        static let configuration = CommandConfiguration(abstract: "Remove the partial destination copy of a migration interrupted before verification; refuses after verification; the source is never touched.")
        @Argument var operationID: String
        func run() throws { try MigrationEngine().abort(operationID: operationID); print("Aborted \(operationID). See `journal` for what was removed; the source was not touched.") }
    }
}
