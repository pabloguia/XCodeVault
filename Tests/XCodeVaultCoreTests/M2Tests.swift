...
final class RuntimeOperationsTests: XCTestCase {
    ...
    func testImportPreflightEnforcesStagingSpace() throws {
        ...
        let journal = Journal(url: URL(fileURLWithPath: t.path + "/j.jsonl"))
        let tight = RuntimeOperations(runner: FakeRunner(responses: [:]), journal: journal, xcode: xcode26, host: host(free: 1_500_000, arm: true))
        XCTAssertThrowsError(try tight.preflightImport(dmg: dmg))
        let ok = RuntimeOperations(runner: FakeRunner(responses: [:]), journal: journal, xcode: xcode26, host: host(free: 3_000_000, arm: true))
